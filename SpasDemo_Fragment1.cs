/// <summary>
/// Example code to send a command to a Spiroson-AS sensor.
/// The code is just a fragment and not compilable!
/// .Net 2.0 required (Download over Windows-Update)
/// </summary>
/// <remarks>
...
namespace SpasDemo
{
  ...
  //.Net 2.0 SerialPort class used. Defined e.g. in the Form-Designer.
  ...
  {
    ...
    //Send a command to the Spiroson-AS sensor. Start the sensor (Data-Sampling)
    //Implementation seen below.
    SendData(SPIROSON_CMD.Start_Peak_Sampling);//Command 23
    ...
  }
  ...
  private void SendData(byte[] byNewData)
  {
    ...
    //Add data if available. No data if only a command with no parameters is sent (e.g. Start_Peak_Sampling)
    ...
    //Check if null befor sending the data.
    if (serialPort1 != null) serialPort1.Write(byData,0,byData.Length);
  }
  ...

}//end namespace SpasDemo

namespace Tools
{
  /// <summary>
  /// CRC calculation class: polinominal CRC_CCITT, initial value=0, start polynominal 0x1021,
  /// reverse data bytes and reverse CRC result befor final XOR.
  /// 
  /// Limitation: for data '0', the CRC is zero. 
  /// </summary>
  /// <remarks>
  /// Copyright © 2006 ndd Medizintechnik AG 
  /// </remarks>
  public class CCrc
  {
    ...
    /// <summary>
    /// CRC16 CCITT table for faster use
    /// </summary>
    private static ushort[] CRC16_table_refl =
    {
    ...
    /// </summary>
    /// <param name="byData">data byte array</param>
    /// <param name="len">data array length, if Lenght == zero, 0 is returned</param>
    /// <returns>CRC value</returns>
    public static int CalculateCRC(byte[] byData, int len)
    {
      int i, k = 0;
      int wCrctt = 0;
      ...
      while ((len--) > 0) wCrctt = (wCrctt >> 8) ^ CRC16_table_refl[(wCrctt & 0xff) ^ byData[k++]];
      return (wCrctt);
    }
  }//end class CCrc
}//end namespace
